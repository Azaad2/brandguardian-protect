  ...
  try {
    ...
    try {
      ...
      if (!profile) {
        ...
        // Try to get user from auth.users to see if user exists
        const { data: authUser, error: authError } = await supabaseClient.auth.admin.getUserById(user_id)
        console.log('Auth user check:', { authUser: !!authUser, authError })
        
        return new Response(
          JSON.stringify({ 
            error: 'User profile not found',
            details: 'Please ensure your profile is complete and try again',
            debug: { user_id, profile_exists: false, auth_user_exists: !!authUser }
          }),
          {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 400,
          ...
          if (!customerResponse.ok) {
            ...
            return new Response(
              JSON.stringify({ 
                ...
                debug: { customerPayload, status: customerResponse.status }
              }),
              {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                status: 400,
        ...
        if (!orderResponse.ok) {
          ...
          return new Response(
            JSON.stringify({ 
              ...
              debug: { orderPayload, status: orderResponse.status }
            }),
            {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              status: 400,
      ...
      console.error('Profile fetch threw error:', profileFetchError)
      return new Response(
        JSON.stringify({ 
